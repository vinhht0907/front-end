<template>
  <the-portlet title="Topic List">
    ...
    <topics-modal ref="modal" :on-action-success="updateItemSuccess" />
  </the-portlet>
</template>
...
import TopicsModal from '~/features/topic/TopicsModal'
import { notifyDeleteSuccess, notifyTryAgain } from '~/utils/bootstrap-notify'
import { TOPIC_VIEW } from '~/constants/permissions'

export default {
  name: 'Topic',
  permission: TOPIC_VIEW,
  components: { TopicsModal, DataTable, ThePortlet },
  head() {
    return {
  ...
  methods: {
    ...
    deleteItem(table, rowData) {
      this.$bvModal
        ...
        .then(async (value) => {
          if (value) {
            await this.$axios.delete('/topic/' + rowData._id)
            notifyDeleteSuccess('Topic')
            this.$refs.table.reload()
          }
        })
        .catch(() => {
