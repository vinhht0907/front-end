<template>
  <div class="list-item-script">
    <b-form-group label="Danh sách từ" class="mb-2">
      <el-collapse v-model="activeNames">
        ...
        >
          <transition-group type="transition" name="list-answer">
            ...
            >
              <template slot="title">
                <span>Từ {{ index + 1 }} / {{ innerValue.length }}</span>
    ...
    <b-button @click="addQuestion" block variant="primary"
      ><i class="la la-plus"></i> Thêm từ</b-button
    >
  </div>
</template>
...
import FindTheWordsItem from '~/components/elements/script/partial/primarySchool/FindTheWordsItem'

export default {
  name: 'FindTheWordsList',
  components: {
    FindTheWordsItem,
    draggable
  },
