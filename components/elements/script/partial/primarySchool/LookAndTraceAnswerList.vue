<template>
  <div class="list-item-script">
    <b-form-group label="Danh sách ảnh và câu hỏi" class="mb-2">
      <el-collapse v-model="activeNames">
        ...
        >
          <transition-group type="transition" name="list-answer">
            ...
            >
              <template slot="title">
                <span>Câu hỏi {{ index + 1 }} / {{ innerValue.length }}</span>
    ...
    <b-button @click="addQuestion" block variant="primary"
      ><i class="la la-plus"></i> Thêm</b-button
    >
  </div>
</template>
...
import LookAndTraceAnswerItem from '~/components/elements/script/partial/primarySchool/LookAndTraceAnswerItem'

export default {
  name: 'LookAndTraceAnswerList',
  components: {
    LookAndTraceAnswerItem,
    draggable
  },
  filters: {
    truncate(value) {
