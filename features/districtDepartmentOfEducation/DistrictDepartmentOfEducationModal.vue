<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa phòng giáo dục' : 'Thêm phòng giáo dục'"
  ...
  >
    <validation-observer ref="observer" class="kt-form">
      ...
      <b-text-input-with-validation
        ...
        :required="true"
        :error="vForm.errors.get('short_name')"
        label="Tên hiển thị"
...
export default {
  name: 'DistrictDepartmentOfEducationModal',
  components: {
    ValidationObserver,
  ...
  methods: {
      ...
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL +
            '/district-department-of-educations/add'
        )

        notifyAddSuccess('phòng giáo dục')
    ...
    async updateItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL +
            '/district-department-of-educations/edit'
        )

        notifyUpdateSuccess('phòng giáo dục')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
