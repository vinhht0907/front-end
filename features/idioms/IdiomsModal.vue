<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa thành ngữ' : 'Thêm thành ngữ'"
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(this.$axios.defaults.baseURL + '/idioms/add')

        notifyAddSuccess('Thành ngữ')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(this.$axios.defaults.baseURL + '/idioms/edit')

        notifyUpdateSuccess('Thành ngữ')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
