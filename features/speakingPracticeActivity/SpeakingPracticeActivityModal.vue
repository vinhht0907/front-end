<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa activity luyện nói' : 'Thêm activity luyện nói'"
  ...
  >
    <validation-observer ref="observer" class="kt-form">
      <b-text-input-with-validation
        ...
        label="Tên activity luyện nói"
        placeholder="Tên activity luyện nói"
      ...
      <select2-with-validation
        ...
        placeholder="Chọn bài luyện nói..."
        label="Bài luyện nói"
        data-vv-as="Bài luyện nói"
        id-field="_id"
        text-field="name"
        ajax="/speaking-practice/suggest"
        name="part_id"
        rules="required"
      />
...
export default {
  name: 'SpeakingPracticeActivityModal',
  components: {
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/speaking-practice-activity/add'
        )

        notifyAddSuccess('Activity luyện nói')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/speaking-practice-activity/edit'
        )

        notifyUpdateSuccess('Activity luyện nói')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
