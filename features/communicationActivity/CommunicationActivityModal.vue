<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa bài tập giao tiếp' : 'Thêm bài tập giao tiếp'"
  ...
  >
    <validation-observer ref="observer" class="kt-form">
      ...
      <select2-with-validation
        ...
        :error="vForm.errors.get('communication')"
        placeholder="Chọn giao tiếp..."
        label="Giao tiếp"
        data-vv-as="Giao tiếp"
        id-field="_id"
        text-field="name"
        ajax="/communication/suggest"
        name="communication"
        rules="required"
      />
...
export default {
  name: 'CommunicationActivityModal',
  components: {
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/communication-activity/add'
        )

        notifyAddSuccess('Bài tập giao tiếp')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/communication-activity/edit'
        )

        notifyUpdateSuccess('Bài tập giao tiếp')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
