<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa bài hát tiếng anh' : 'Thêm bài hát tiếng anh'"
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(this.$axios.defaults.baseURL + '/song/add')

        notifyAddSuccess('Bài hát tiếng anh')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(this.$axios.defaults.baseURL + '/song/edit')

        notifyUpdateSuccess('Bài hát tiếng anh')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
