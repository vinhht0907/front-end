<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa bài tập đọc' : 'Thêm bài tập đọc'"
  ...
  >
    <validation-observer ref="observer" class="kt-form">
      ...
      <select2-with-validation
        ...
        :error="vForm.errors.get('reading')"
        placeholder="Chọn bài đọc..."
        label="Bài đọc"
        data-vv-as="Bài đọc"
        id-field="_id"
        text-field="name"
        ajax="/reading/suggest"
        name="reading"
        rules="required"
      />
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/reading-activity/add'
        )

        notifyAddSuccess('Bài tập đọc')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/reading-activity/edit'
        )

        notifyUpdateSuccess('Bài tập đọc')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
