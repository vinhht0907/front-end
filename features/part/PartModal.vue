  ...
  >
    <validation-observer ref="observer" class="kt-form">
      <b-text-input-with-validation
        ...
        label="Tên lesson part"
        placeholder="Tên lesson part"
...
export default {
  ...
  watch: {
    'form.course_id'(newVal, oldVal) {
      if (oldVal !== null && newVal && newVal._id !== oldVal._id) {
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(this.$axios.defaults.baseURL + '/part/add')

        notifyAddSuccess('Lesson part')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(this.$axios.defaults.baseURL + '/part/edit')

        notifyUpdateSuccess('Lesson part')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
