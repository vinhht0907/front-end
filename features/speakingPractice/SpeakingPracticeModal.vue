<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa bài luyện nói' : 'Thêm bài luyện nói'"
  ...
  >
    <validation-observer ref="observer" class="kt-form">
      <b-text-input-with-validation
        ...
        label="Tên bài luyện nói"
        placeholder="Tên bài luyện nói"
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/speaking-practice/add'
        )

        notifyAddSuccess('Bài luyện nói')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/speaking-practice/edit'
        )

        notifyUpdateSuccess('Bài luyện nói')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
