<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa bài tập ngữ pháp' : 'Thêm bài tập ngữ pháp'"
  ...
  >
    <validation-observer ref="observer" class="kt-form">
      ...
      <select2-with-validation
        :required="true"
        v-model="form.grammar_id"
        :error="vForm.errors.get('grammar_id')"
        placeholder="Chọn ngữ pháp..."
        label="Ngữ pháp"
        data-vv-as="Ngữ pháp"
        id-field="_id"
        text-field="name"
        ajax="/grammar/suggest"
        name="grammar_id"
        rules="required"
      />
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/grammar-activity/add'
        )

        notifyAddSuccess('Bài ngữ pháp')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/grammar-activity/edit'
        )

        notifyUpdateSuccess('Bài ngữ pháp')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
