<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa bài tập truyện' : 'Thêm bài tập truyện'"
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/story-activity/add'
        )

        notifyAddSuccess('Bài tập truyện')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/story-activity/edit'
        )

        notifyUpdateSuccess('Bài tập truyện')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
