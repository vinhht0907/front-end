<template>
  <b-modal
    ...
    :title="
      isEdit
        ? 'Sửa activity bài hát tiếng anh'
        : 'Thêm activity bài hát tiếng anh'
    "
  ...
  >
    <validation-observer ref="observer" class="kt-form">
      ...
      <select2-with-validation
        ...
        :error="vForm.errors.get('song_id')"
        placeholder="Chọn bài hát tiếng anh..."
        label="Bài hát tiếng anh"
        data-vv-as="Bài hát tiếng anh"
        id-field="_id"
        text-field="name"
        ajax="/song/suggest"
        name="song_id"
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/song-activity/add'
        )

        notifyAddSuccess('Activity bài hát tiếng anh')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/song-activity/edit'
        )

        notifyUpdateSuccess('Activity bài hát tiếng anh')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
