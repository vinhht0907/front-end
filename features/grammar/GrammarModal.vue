<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa ngữ pháp' : 'Thêm ngữ pháp'"
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(this.$axios.defaults.baseURL + '/grammar/add')

        notifyAddSuccess('Ngữ pháp')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(this.$axios.defaults.baseURL + '/grammar/edit')

        notifyUpdateSuccess('Ngữ pháp')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
