<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa bài tập bộ từ' : 'Thêm bài tập bộ từ'"
  ...
  >
    <validation-observer ref="observer" class="kt-form">
      ...
      <select2-with-validation
        :required="true"
        v-model="form.word_group_id"
        :error="vForm.errors.get('word_group_id')"
        placeholder="Chọn bộ từ..."
        label="Bộ từ"
        data-vv-as="Bộ từ"
        id-field="_id"
        text-field="name"
        ajax="/word-group/suggest"
        name="word_group_id"
        rules="required"
      />
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/word-group-activity/add'
        )

        notifyAddSuccess('Bài tập bộ từ')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/word-group-activity/edit'
        )

        notifyUpdateSuccess('Bài tập bộ từ')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
