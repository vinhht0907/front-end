<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa bài tự luận' : 'Thêm bài tự luận'"
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/writing-test/add'
        )

        notifyAddSuccess('Bài tự luận')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/writing-test/edit'
        )

        notifyUpdateSuccess('Bài tự luận')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
