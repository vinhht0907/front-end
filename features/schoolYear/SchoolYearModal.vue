  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(this.$axios.defaults.baseURL + '/school-year/add')

        notifyAddSuccess('năm học')
    ...
    async updateItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/school-year/edit'
        )

        notifyUpdateSuccess('năm học')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
