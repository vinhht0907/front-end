<template>
  <div class="list-collapse">
    <b-form-group :label="label" class="mb-0" label-class="required">
      <el-collapse v-model="activeNames">
        ...
        >
          <transition-group type="transition" name="list-answer">
            <el-collapse-item
              v-for="(review, index) in innerValue"
              :key="review.key"
              :name="review.key"
            >
              <template slot="title">
                <span>
                  {{
                    `Nhận xét ${index + 1} / ${innerValue.length} ${
                      review.pronunciation_score && review.texts.length > 0
                        ? `- ${review.pronunciation_score.name} -
                  ${review.texts[0].text}`
                        : ''
                    }` | truncate
                  }}</span
                ...
                <span v-show="showAction" class="collapse-action">
                  ...
                  <a
                    ...
                    ><i
                      @click.self="removeItem(review)"
                      class="la la-trash-o"
                    ></i
                  ></a>
                </span>
              </template>
              <pronunciation-audio-item
                v-if="activeNames.includes(review.key)"
                v-bind="$attrs"
                :emojis="emojis"
                :type="type"
    ...
    <b-button @click="addItem" block variant="primary" size="sm"
      ><i class="la la-plus"></i> Thêm nhận xét</b-button
    >
  </div>
</template>
...
import PronunciationAudioItem from './CommentOnPronunciationItem'
