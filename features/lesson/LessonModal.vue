<template>
  <b-modal
    ...
    :title="isEdit ? 'Cập nhật quản lý bài học' : 'Thêm mới'"
  ...
  >
    <validation-observer ref="observer" class="kt-form">
      ...
      <b-summernote-with-validation
        ...
        :required="true"
        :error="vForm.errors.get('summary')"
        label="Tóm tắt bài học (shift+enter để xuống dòng trong code block)"
      ...
      <checkboxes-with-validation
        ...
        :error="vForm.errors.get('is_exam')"
        label="Kiểm tra"
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(this.$axios.defaults.baseURL + '/lesson/add')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(this.$axios.defaults.baseURL + '/lesson/edit')
