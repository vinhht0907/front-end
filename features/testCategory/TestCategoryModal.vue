<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa danh mục bài thi' : 'Thêm danh mục bài thi'"
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/test-category/add'
        )

        notifyAddSuccess('Danh mục bài thi')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(
          this.$axios.defaults.baseURL + '/test-category/edit'
        )

        notifyUpdateSuccess('Danh mục bài thi')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
