<template>
  <b-modal
    ...
    :title="isEdit ? 'Sửa bài đọc' : 'Thêm bài đọc'"
  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(this.$axios.defaults.baseURL + '/reading/add')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(this.$axios.defaults.baseURL + '/reading/edit')
