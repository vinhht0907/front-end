  ...
  methods: {
    ...
    async addItem() {
      try {
        ...
        await this.vForm.post(this.$axios.defaults.baseURL + '/story/add')

        notifyAddSuccess('Truyện')
    ...
    async updateItem() {
      try {
        this.vForm = new Form(this.form)
        await this.vForm.post(this.$axios.defaults.baseURL + '/story/edit')

        notifyUpdateSuccess('Truyện')
        this.$refs.modal.hide()
        this.onActionSuccess()
      } catch (e) {
        if (e.response) {
